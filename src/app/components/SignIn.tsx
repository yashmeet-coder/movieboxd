...
const SignIn = ({setShowSignIn}:{setShowSignIn:(value:boolean)=>void}) => {
  ...
  return (
    <div>
      <div
          className={`absolute top-0 md:right-4 z-10 right-0 w-screen md:w-auto flex-col md:flex md:flex-row gap-4 items-center pl-[30px] px-2 py-4 bg-[#14181C]`}
        >
          <RiCloseLargeFill
            className="absolute top-2 md:top-10 right-2 md:left-2 w-4 h-4 text-[#8696A7] hover:text-white cursor-pointer"
            onClick={() => setShowSignIn(false)}
          />
          <form id="signinform" action={handleSignIn} className="flex flex-col gap-4 md:flex-row" >
